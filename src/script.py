...
# train[['device_id','fold']].to_csv(os.path.join(datadir,"folds_10.csv"),index=False)
...
pred_train_df.to_csv('../data/keras_pred_train_bags5_wEvents_allData_20160824.csv', index=True, index_label='device_id')
pred_test_df.to_csv('../data/keras_pred_test_bags5_wEvents_allData_20160824.csv', index=True, index_label='device_id')

print(pred_test_df.head(1))

#generate prediction:
# submission = pd.DataFrame(pred_test[:,0:12], index = gatest.index, columns=targetencoder.classes_)
# submission.to_csv('../data/keras_cv10_plus_regression_80_reg.csv',index=True)

submission = pd.DataFrame(pred_test[:,12:25], index = gatest.index, columns=targetencoder.classes_)
submission.to_csv('../data/keras_cv10_with_bags5_wEvents_AllData.csv',index=True)
